""" Prediction using Regression Methods
    ...
    Usage:
      $ python -m algo.cb.gbrt [-r <learning_rate>] [-t <num_trees>] 
        [-d <max_depth>] [-l <loss_func>] [-p <subsample>] [-m <max_feat>]
...
def load_args():
  ...
  while i < len(argv): 
    if argv[i] == '-r':
      ...
      print argv[i]
      print argv[i+1]
      print ('Usage $ python -m algo.cb.gbrt [-r <learning_rate>]'
...
def main():
  ...
  for i in xrange(NUM_SETS):
    for j in xrange(REP):
      ...
      if _BIAS:
        bias = BiasModel()
        train = bias.fit_transform(train)
      ...
      if _BIAS:
        bias.add_bias(val, pred)
      output = open('%s/gbrt-r:%f,t:%d,d:%d,l:%s,p:%f,m:%s,s:%s,f:%s,b:%s-%d-%d.dat' % 
